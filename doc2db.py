...
conn = sqlite3.connect('2017e.db') #db exclusive for 5a
c=conn.cursor()
...
c.execute('''SELECT * FROM slot''') #tried to copy the original 'slot' table into 'slot' for 5A, but not working!!



wordDoc = Document("Docs/5b.docx") #doc containing TT of 5A only so that its easier to access subjects individually
text = []
time_tables = []
class_rooms = []
subjects = []
...
#print time_tables
TABLE = iterutils.chunked(time_tables, 13) #creates a 2D list from the availabe 1D list
